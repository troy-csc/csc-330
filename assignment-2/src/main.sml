...
fun test_set_ops() =
    let
      ...
      val _ = test(sb EXCEPT sb IDENTICAL se, "testing except")
...
fun test_pairs() =
    let
      ...
      (*val _ = print("Result: " ^ str_set(sa, str_pair_int) ^ "\n")*)
    in
      ()
    end
...
fun test_lists() =
    let
      ...
      (*val _ = print("Result: " ^ str_set(sa, str_list_int) ^ "\n")*)
    in
      ()
    end
...
fun test_lists_strings() =
    let
      ...
      (*val _ = print("Expected: \t{[]:[,abc]:[Canada,Japan,Brazil]:[daniel,robert]}\n")
      val _ = print("Result: \t" ^ str_set(sa, str_list_string) ^ "\n")*)
    in
      ()
    end
...
fun troy_tests () =
	...
	val se = EmptySet Int.compare
	val s0 = se ++ 5 -- 5
	val s1 = Set([], Int.compare)
	val _ = test(is_empty_set s0, "testing empty set after removal")
	val _ = test(is_empty_set s1, "testing set with empty list for emptiness")
    in
	()
    end
